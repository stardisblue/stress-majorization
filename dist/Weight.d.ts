 ...
 */
export declare const WeightFactory: <T>(distance: PairFn<T>) => {
    one: () => 1;
    distance: PairFn<T>;
    distanceInversePow: (i: T, j: T) => number;
    distanceInverse2Pow: (i: T, j: T) => number;
    exponentialInverseDistance: (i: T, j: T) => number;
};
